...
function Enemy(pos) {
    ...
    this.basePos = this.pos = pos.plus(new Vector(-1.2, -1.0));
    this.size = new Vector(0.8, 1.20);
    this.jump = Math.random() * Math.PI * 3;
...
Level.prototype.obstacleAt = function (pos, size) {
    ...
    //Consider the sides and top/bottom of the level as walls
...
Enemy.prototype.act = function (step) {
    this.jump += step * jumpSpeed;
    var jumpPos = Math.cos(this.jump) * jumpDist;
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  var killed = new Vector(5, 15);

  //Find if there is an obstacle at the newPos
  var obstacle = level.obstacleAt(newPos, this.size);
  
  //Move if there's not a obstacle
  if (obstacle == "lava") {
      this.pos = killed;
  }
...
Player.prototype.moveY = function (step, level, keys) {
  ...
  //if they are touching some obstacles
 
  if (obstacle) {
      //If they touch lava reset to start of level
      if (obstacle == "lava")
          this.pos = killed;
